...
router.post('/patient/:phone/restore-shield', async (req, res) => {
  try {
    ...
    // Hard cap: 3 shields per month per patient regardless of source.
    // Coach restoration refunds 1 used shield (decrements `used`), it doesn't push the cap above 3.
    const used = streak.shields?.used || 0;
    if (used <= 0) {
      return res.status(400).json({ error: 'Patient already has the full 3 shields this month' });
    }

    await Streak.findByIdAndUpdate(streak._id, {
      'shields.used': used - 1,
      'shields.monthly': 3,
      'shields.lastResetMonth': currentMonth,
