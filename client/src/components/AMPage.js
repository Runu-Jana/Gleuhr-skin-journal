  ...
  useEffect(() => {
    ...
    const loadToday = async () => {
      try {
        ...
        // Only load today's check-in if it exists and is from today
        if (today && today.date === new Date().toISOString().split('T')[0]) {
          // Only set as submitted if the check-in is actually completed
          // Don't automatically set buttons to true for new users
          ...
          // Only mark as submitted if both AM routine and sunscreen are explicitly set
          // This prevents old data from marking new users as completed
          if (today.amRoutine === true && today.sunscreen === true) {
            setHasSubmitted(true);
          }
        } else {
          // Reset to default state for new users or no check-in found
          setAmRoutine(false);
          setSunscreen(false);
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-4 border-b border-gray-100">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform absolute top-1 ${
                      amRoutine ? 'translate-x-7' : 'translate-x-1'
                    }`} />
                  </button>
              ...
              <div className="flex items-center justify-between py-4 border-b border-gray-100">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform absolute top-1 ${
                      sunscreen ? 'translate-x-7' : 'translate-x-1'
                    }`} />
                  </button>
                  <span className={`text-sm font-medium ${
